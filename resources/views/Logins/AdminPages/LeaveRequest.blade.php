...
<body>
    ...
    <div class="table-container">
      <div class="status dash-center">
          <a class="submit-btn dash-btn" href="{{ route('LeaveExtension') }}" style="text-decoration:none">Leave Extension Request <span class="leave_count">{{$count}}</span></a>
      </div>
              ...
              @if($stud->faculty_adv==1)
                  <button class="accordion-button custom-button faculty-approved" type="button" data-bs-toggle="collapse" data-bs-target="#collapse{{ $loop->iteration }}" aria-expanded="true" aria-controls="collapse{{ $loop->iteration }}">
                    <div class="button-content">
                      ...
                      <span class="received-time">&nbsp;&nbsp;Received On: {{ Carbon::parse($stud->created_at)->timezone('Asia/Kolkata')->format('d-m-Y h:i a') }}</span>
                    </div>
                  </button>
              @else
